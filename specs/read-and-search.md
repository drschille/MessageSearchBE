...
**Document lifecycle:** A document may be created and updated multiple times, but a snapshot is only created when the document is published (e.g., via `POST /v1/documents` with a publish flag). Snapshots are immutable and represent the published state of a document at a specific point in time. Documents may exist without any snapshots until they are published for the first time.
...
```
The service unwraps `total` and `results`, ensuring API responses carry overall hit counts alongside paginated data. Note that `total` reflects only documents matching either text or vector criteria; documents with zero scores are excluded.

> **Note:** The `snippet` field in each search result is generated by the application layer after the SQL query returns. The service constructs `snippet` by extracting and highlighting relevant fragments from the document body that match the search query terms.
...
- **Validation & errors:** 400 for malformed queries or missing weights, 401/403 for auth failures, 404 for missing documents, 429 for rate limits, 500 with correlation ID for unexpected errors.
...
- `/v1/answer`: p95 < 3 s assuming upstream model latency < 2.5 s; degrade gracefully by returning search hits + “answer unavailable”.
  - _Note: The 2 s timeout applies specifically to the embeddings API during `/v1/search`. The 2.5 s latency target applies to upstream chat/generation models during `/v1/answer` operations. These thresholds are independent and should not be conflated._
